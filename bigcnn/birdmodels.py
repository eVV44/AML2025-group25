...
class CosineClassifier(nn.Module):
    ...
    # Adjusted to ArcFace instead of standard cosine classifier
    def forward(self, embeddings: torch.Tensor, labels: torch.Tensor | None = None):
        cos_theta = calc_cosine_similarity(embeddings, self.prototypes)
...
class BirdClassifier(nn.Module):
    def __init__(self, num_classes: int = 200, attribute_dim: int = 312, embedding_dim: int = 512, 
                 ...
                 classifier_scale: float = 10.0, classifier_margin: float = 0.0):
        super().__init__()
